...
>
> **第8月 W2 关键数据**：真实时相对（同 tile 49QGE，2023-07-08 vs 2025-07-27，2 年间隔）——
> 两景 700×1100 同 CRS 同 transform，**像素级天然对齐**，无需配准。
> 评估核心：真实数据**无人工 GT** → 改用"方法间一致率"+ 水域稳定性自检（非准确率）。
> ① **光谱差分**：变化占比 **67.6%**（过度检测，潮汐/物候/大气全是噪声）—— 与 W1 合成版结论一致
> ② **U-Net 分类后比较**（unet_finetuned.pt，W2 真实微调权重）：**11.9%**，集中在建成区边缘
> ③ **伪标签弱参考**（NDWI/NDVI 阈值法，与微调同规则）：**11.2%**
> ④ **方法间一致率**：U-Net vs 伪标签 **94.6%**（两独立方法高度吻合 → 互证可信）；U-Net vs 光谱差分 42.1%
> ⑤ **水域稳定性自检**：98,415 深水像素中仅 **1.51%** 被判水→城翻转 → 真实域分类可靠（红旗④解除）
> ⑥ **类别转换**：水→水 109,284（海岸稳定），变化集中在城↔植（23,802 植→城 + 10,620 城→植）
> ⑦ 关键工程：扩展 `satellite_download.py` 加 `--tile/--cloud/--cloud` 过滤，支持 49QGE 等指定 tile 同视角下载
> ⚠ **诚实红旗**：① 弱参考是代理非真值 ② 2023 景云 27.5%（云影/薄云可能漏判）③ 潮汐/物候差异仍可能制造假变化
> 数据：`data/cogs/szbay_real_20230708.tif`（新增，3.4MB，单景 49QGE） + 已有 `szbay_real_20250727.tif`
