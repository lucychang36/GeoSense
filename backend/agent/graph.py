...
SPATIAL_SYSTEM_PROMPT = (
    ...
    "6. 涉及具体地理区域（区/市/省/任意地名）的数据可用性不确定时，"
    "先调 data_inventory_tool 探查（region 传地名，或 bbox 传 \"west,south,east,north\" 支持全球）："
    "有数据→按返回的两期文件继续分析；无数据→按返回的 recommendation 诚实告知缺口并推荐替代区域，"
    "禁止编造影像文件名、日期或数字；\n"
    "7. 回答时给出关键数值、单位，并说明用了哪些工具。"
)
